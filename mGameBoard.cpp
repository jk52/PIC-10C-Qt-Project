...
mGameBoard::mGameBoard(QWidget *parent) :
    QWidget(parent), board_size(10)
{
    ...
    labels = new customLabelm*[board_size*board_size];


    //i=y coordinate from (0,9), j=x coordinate from (0,9)
    for(int i=0;i<board_size;i++) {
         for(int j=0;j<board_size;j++) {
            labels[i*board_size+j] = new customLabelm(this);
            ...
            labels[i*board_size+j]->setStyleSheet("customLabelm { background-color : white; color : black; border-style: outset; border-color : gray; border-width : 4px; }");
...
void mGameBoard::closeEvent(QCloseEvent *e) {
...
void mGameBoard::setQuitWidget(QuitWidget* quitWidget) {

    quit = quitWidget;

}

void mGameBoard::setMainWindow(MainWindow* mainWindow) {
    gameScreen = mainWindow;

}

void mGameBoard::paintEvent(QPaintEvent *e){
    if(current_level==1){
