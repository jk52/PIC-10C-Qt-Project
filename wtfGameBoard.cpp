...
wtfGameBoard::wtfGameBoard(QWidget *parent) :
    QWidget(parent), board_size(10)
{
    ...
    labels = new customLabelwtf*[board_size*board_size];


    //i=y coordinate from (0,9), j=x coordinate from (0,9)
    for(int i=0;i<board_size;i++) {
         for(int j=0;j<board_size;j++) {
            labels[i*board_size+j] = new customLabelwtf(this);
            ...
            labels[i*board_size+j]->setStyleSheet("customLabelwtf { background-color : white; color : black; border-style: outset; border-color : gray; border-width : 4px; }");
...
void wtfGameBoard::closeEvent(QCloseEvent *e) {
...
void wtfGameBoard::setQuitWidget(QuitWidget* quitWidget) {

    quit = quitWidget;

}

void wtfGameBoard::setMainWindow(MainWindow* mainWindow) {
    gameScreen = mainWindow;

}

void wtfGameBoard::paintEvent(QPaintEvent *e){
    if(current_level==1){
...
void wtfGameBoard::moveMonsters(){
...
void wtfGameBoard::upgradeTowers(){
