...
hGameBoard::hGameBoard(QWidget *parent) :
    QWidget(parent), board_size(10)
{
    ...
    labels = new customLabelh*[board_size*board_size];


    //i=y coordinate from (0,9), j=x coordinate from (0,9)
    for(int i=0;i<board_size;i++) {
         for(int j=0;j<board_size;j++) {
            labels[i*board_size+j] = new customLabelh(this);
            ...
            labels[i*board_size+j]->setStyleSheet("customLabelh { background-color : white; color : black; border-style: outset; border-color : gray; border-width : 4px; }");
...
void hGameBoard::closeEvent(QCloseEvent *e) {
...
void hGameBoard::setQuitWidget(QuitWidget* quitWidget) {

    quit = quitWidget;

}

void hGameBoard::setMainWindow(MainWindow* mainWindow) {
    gameScreen = mainWindow;

}

void hGameBoard::paintEvent(QPaintEvent *e){
    if(current_level==1){
